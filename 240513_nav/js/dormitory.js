...
let weeklyReservations;          //미리 정해진 요열별 예약 데이터
...
const setPage = (page) => { //함수를 만든거임
    ...
    if (page === 2){ //시간선택:세탁기,시간
        initWashingmachineTime();
        //선택한 날짜의 요일 구하자
        //그 요일의 미리 예약된 세탁기오 파악하자
        //예약된게 있으면select 목록에서 빼자
        // 그 요일의 미리 예약된 세탁기와 시간이 다 차면,그 세탁기selct목록에서 뺴자
     ...
     //선택한 날짜의 요일 구하자
        ...
        const initTime = () => {
            ...
            allWashingmachineTime[selectionWashingmachine].forEach((time) => {
                ...
                newOption.textContent = allData.Time[time] //"2" -> allData.time["2"] (8시 10분~ 9시20분).time - > allData.time[time]
